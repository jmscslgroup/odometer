...
// C/C++ source code generated on : Mon Nov 06 21:54:38 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"odometer\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(odometer_M, (NULL));
        odometer_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(odometer_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(odometer_M)
                == (NULL)) || rtmGetStopRequested(odometer_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(odometer_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
